...
class Frcnn_Model:
	def __init__(self):
		...
		config_output_filename = self.config_filename
	...
	def get_bouding_boxes(self, img_arr):
		all_imgs = []
		...
		img = self.convertPILtoCV(img_arr)
		...
		R = roi_helpers.rpn_to_roi(Y1, Y2, self.C, K.common.image_dim_ordering(), overlap_thresh=0.7)
		...
		for key in bboxes:
			...
			for jk in range(new_boxes.shape[0]):
				...
				#cropped_image = img[real_x1:real_x2, real_y1:real_y2]

				crop_x1 = max(0, real_x1)
				crop_x2 = min(len(img[0,:]), real_x2)
				crop_y1 = max(0, real_y1)
				crop_y2 = min(len(img[:,0]), real_y2)

				coord = (crop_x1, crop_y1, crop_x2, crop_y2)

				all_dets.append((coord,100*new_probs[jk]))

			#cropped_frame = img[crop_y1:crop_y2, crop_x1:crop_x2]

			#cv2.imwrite('./results_imgs/cropped_{}_{}.png'.format(idx, crop_num), cropped_frame)
			#crop_num = crop_num + 1

		#print(all_dets) [((x1, y1, x2, y2), Confidence), ()]
		return all_dets
	...
	def get_crops(self, image, boxes):
		crops = []
		cv_img = self.convertPILtoCV(image)
		for box in boxes:
			x1, y1, x2, y2 = box[0], box[1], box[2], box[3]
			cv_crop = cv_img[x1:x2, y1:y2]
